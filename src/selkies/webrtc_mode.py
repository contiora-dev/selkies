...
from .settings import (
    settings_webrtc as settings,
    AppSettings,
    FINAL_SETTING_DEFINITIONS_WEBRTC as SETTING_DEFINITIONS,
)
...
class WebRTCApp:
    ...
    async def initialize_components(self) -> None:
        ...
        self.media_pipeline = MediaPipelinePixel(
            ...
            rc_mode=RateControlMode(self.args.rate_control_mode)
            if self.args.enable_rate_control
            else RateControlMode.CBR,
        )
        ...
        # Signaling server
        self.signaling_server = self.create_signaling_server(
            rtc_config, self.monitoring_utils_used
        )
    ...
    def create_signaling_server(
        self, rtc_config: Any, mon_utils_used: Dict[str, bool]
    ) -> WebRTCSimpleServer:
        ...
        options.turn_shared_secret = (
            self.args.turn_shared_secret if using_hmac_turn else ""
        )
    ...
    def setup_callbacks(self) -> None:
        ...
        self.input_handler.on_cursor_change = (
            lambda data: self.rtc_app.send_cursor_data(data)
        )
        ...
        self.input_handler.on_mouse_pointer_visible = (
            lambda v: self.media_pipeline.set_pointer_visible(v)
        )
        self.input_handler.on_clipboard_read = (
            lambda d, t: self.rtc_app.send_clipboard_data(d, t)
        )
        self.input_handler.on_set_fps = self.handle_fps_change
        self.input_handler.on_client_fps = (
            lambda f: self.metrics.set_fps(f) if self.metrics else None
        )
        self.input_handler.on_client_latency = (
            lambda l: self.metrics.set_latency(l) if self.metrics else None
        )
        self.input_handler.on_ping_response = (
            lambda latency: self.rtc_app.send_latency_time(latency)
        )
    ...
    async def shutdown(self) -> None:
        ...
        if self.signaling_server:
            stop_coros.append(
                (
                    _await_with_timeout(
                        self.signaling_server.stop(), "signaling_server", 3.0
                    )
                )
            )
        if self.media_pipeline:
        ...
        if self.metrics:
            stop_coros.append(
                (_await_with_timeout(self.metrics.stop_http(), "metrics", 2.0))
            )
